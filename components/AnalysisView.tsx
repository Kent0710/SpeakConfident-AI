    ...
    <div className="w-full max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            ...
            <p className="text-gray-500 mt-1">Here is how you sounded in your presentation.</p>
        </div>
        <button
          onClick={onReset}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col items-center justify-center">
            <h3 className="text-gray-500 font-medium text-sm uppercase tracking-wider mb-2">Overall Score</h3>
            <div className={`relative flex items-center justify-center w-32 h-32 rounded-full border-8 ${result.overallScore >= 80 ? 'border-green-100' : result.overallScore >= 60 ? 'border-yellow-100' : 'border-red-100'}`}>
                <span className={`text-4xl font-bold ${getScoreColor(result.overallScore)}`}>
                    {result.overallScore}
                </span>
            </div>
            <p className="mt-4 text-center text-gray-600 text-sm italic">"{result.summary}"</p>
        </div>

        {/* Radar Chart */}
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 col-span-1 md:col-span-2 flex flex-col">
            <h3 className="text-gray-500 font-medium text-sm uppercase tracking-wider mb-4">Performance Dimensions</h3>
      ...
      {result.visualAnalysis && (
        <div className="bg-indigo-900 rounded-2xl p-6 md:p-8 text-white shadow-xl relative overflow-hidden">
            ...
            <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                     ...
                     <h3 className="text-2xl font-bold">Visual Presence Analysis</h3>
                </div>
                
                <p className="text-indigo-200 mb-8 max-w-3xl text-lg leading-relaxed">{result.visualAnalysis.generalFeedback}</p>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {result.visualAnalysis.metrics.map((metric, idx) => (
                        <div key={idx} className="bg-white/10 rounded-xl p-5 backdrop-blur-sm border border-white/10 hover:bg-white/20 transition-all duration-300">
                            <div className="flex justify-between items-center mb-3">
                                <span className="font-semibold text-lg tracking-wide">{metric.label}</span>
                                <span className={`px-2 py-1 rounded text-xs font-bold ${metric.score >= 80 ? 'bg-green-500/30 text-green-200' : metric.score >= 60 ? 'bg-yellow-500/30 text-yellow-200' : 'bg-red-500/30 text-red-200'}`}>
                                    {metric.score}/100
                                </span>
                            </div>
                            ...
                            <p className="text-sm text-indigo-100/90 leading-relaxed">{metric.feedback}</p>
                        </div>
                    ))}
                </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <h3 className="text-gray-900 font-bold text-lg mb-6">Detailed Breakdown</h3>
            <div className="space-y-6">
                {result.metrics.map((metric) => (
                    <div key={metric.category}>
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-gray-700 font-medium">{metric.category}</span>
                            <span className={`px-2 py-1 rounded text-xs font-bold ${getScoreBg(metric.score)} ${getScoreColor(metric.score)}`}>
                                {metric.score}/100
                            </span>
                        </div>
        ...
        <div className="space-y-6">
            <div className="bg-green-50 rounded-2xl p-6 border border-green-100">
                ...
                <ul className="space-y-3">
                    {result.strengths.map((str, idx) => (
                        <li key={idx} className="flex items-start gap-3 text-green-900 text-sm">
            ...
            <div className="bg-orange-50 rounded-2xl p-6 border border-orange-100">
                ...
                <ul className="space-y-3">
                    {result.improvements.map((imp, idx) => (
                        <li key={idx} className="flex items-start gap-3 text-orange-900 text-sm">
            ...
            <div className="bg-indigo-50 rounded-2xl p-6 border border-indigo-100">
                <h3 className="text-indigo-900 font-bold text-lg mb-2">Transcription Snippet</h3>
