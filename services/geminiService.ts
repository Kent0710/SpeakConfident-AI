...
const MODEL_NAME = 'gemini-2.0-flash-exp'; // Supports multimodal input (video/audio)
...
export const analyzePresentation = async (file: File): Promise<AnalysisResult> => {
  if (!GEMINI_API_KEY) {
  ...
  const schemaProperties: any = {
    ...
    summary: { type: Type.STRING, description: "A 2-3 sentence summary of the performance." },
    transcriptionSnippet: { type: Type.STRING, description: "A short snippet of what was said (first few sentences)." },
    metrics: {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
  ...
  const requiredFields = ["overallScore", "summary", "metrics", "strengths", "improvements", "transcriptionSnippet"];

  if (isVideo) {
    schemaProperties.visualAnalysis = {
      type: Type.OBJECT,
      properties: {
        generalFeedback: { type: Type.STRING, description: "Overall feedback on visual presence." },
        metrics: {
          type: Type.ARRAY,
          items: {
            type: Type.OBJECT,
            properties: {
              label: { type: Type.STRING, description: "e.g., Eye Contact, Body Language" },
              score: { type: Type.NUMBER },
              feedback: { type: Type.STRING }
            },
